...
If you are not familier with this eco-system or you find the above procedure hard, don't worry, I have given the detailed and step-wise procedure in later part of this document, which will guide you to make your PIC bootloader experience buttery smooth. 
...
\begin{itemize}
\item PIC18F4550 micro-controller board with \textit{ICSP} connector
\item PICKIT-3 Programmer
\item USB cable to coonect prograamer to PC.
\item Jumper wires / Ribbon Cable to connect PICKIT-3 to micro-controller board.
\item PICKIT-3 PC application
\item Bootloader's Hex File
\item PC/Laptop to run the application.
\end{itemize}
...
\begin{itemize}
\item PIC18F4550 micro-controller with bootloader
\item PC or laptop to install driver
\item USB cable to coonect mico-controller board to PC/Laptop
\end{itemize}
...
\section{About this chapter}
In last Chapter we have installed the drivers, so our micro-controller with USB bootloader will get detected by \textit{Device Manager}.\par

Now we can upload the \textit{Hex} file of the \textit{Firmware} we have written for the \textbf{PIC18F4550} micro-controller. If you don't have the firmware hex file, you can write simple application/firmware in \textbf{MPLAB IDE} and generate the hex file by compiling source code.\par



\section{Requirements}
\begin{itemize}
\item PIC18F4550 micro-controller with USB bootloader
\item USB cable to connect micro-controller board with PC/Laptop
\item Power Adapter for mico-controller board \texttt{[If Required]}
\item \textbf{PDFSUSB} PC Application to upload firmware hex file
\item Firmware hex file to upload
\item PC/Laptop
\end{itemize}
You will not required to install the \textit{PDFSUSB} PC application separately as its get installed along with MCHPFSUSB USB Framework. Please refer the \autoref{chap:MCHPFSUSB} for \textit{MCHPFSUSB USB Framework} installtion procedure if you have not installed it yet.

\section{Procedure}

\begin{enumerate}
\item From your PC/Laptop navigate to \texttt{This PC -> Local Disk(C:) -> Microchip Solutions -> USB Tools -> Pdfusb}

\begin{figure}[H]
\centering
\includegraphics[scale=1.6]{PDFSUSB1.jpeg}
\caption{Locate Application exe file}
\label{PDFSUSB_1}
\end{figure}

\item At this location you can find the \texttt{PDFUSB.exe}  file, as shown in figure \ref{PDFSUSB_1}.

\begin{figure}[H]
\centering
\includegraphics[scale=1.6]{PDFSUSB3.jpeg}
\caption{Select Micro-controller Device}
\label{PDFSUSB_2}
\end{figure}

\item A new window will open, at this window, select the microcontroller device as shown in the figure \ref{PDFSUSB_2}.

\begin{figure}[H]
\centering
\includegraphics[scale=1.6]{PDFSUSB5.jpeg}
\caption{Load Hex File}
\label{PDFSUSB_3}
\end{figure}

\item As shown in figure \ref{PDFSUSB_3}, click on the \textit{Load Hex File} button and select the hex file of your firmware.

\item The hex file contents can be seen in the white text area, situated at the bottom side of the application. Which also ensures that the hex file is uploaded succefully.

\item now click on the \textit{Program Device} button so, the hex file we have selected in prevois step is get uploaded in to the micro-controller.

\item After hex file gets uploaded succesfully, you can click on the \textit{Execute} button from the application or push the \textit{Reset} button on the micro-controller board to start execution of the firmware.
